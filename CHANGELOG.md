...
- **`ybtop histogram` command (offline, no database):** Detects statements whose per-call latency distribution is **bimodal/multimodal** (two or more distinct speed classes) in the **latest** snapshot's **cumulative** totals. Tiered detector (bimodality coefficient → density/peak finding → valley check → Hartigan dip test) with **confidence tiers** (`very_high` / `high` / `moderate` / `unconfirmed`), **Benjamini-Hochberg FDR** correction, and **query-template grouping** (IN-list- and per-call-comment-aware). Reports at the `high` tier and above; the only tuning flag is **`--min-calls`** (default `30`). Delta analysis and interactive tier/flagged filtering live in the viewer's **Latency modes** tab. Requires the optional extra `pip install 'ybtop[histogram]'` (numpy, scipy, and diptest for the confirmatory test).
