...
Optional latency-multimodality detector (for `ybtop histogram`; adds numpy, scipy, and diptest):
...
| **`ybtop histogram --data-dir DIR`** | **Offline, no database.** Analyzes the **`latency_histograms`** stored in the **latest** snapshot (captured with `watch --snapshot-latency-histograms`) to flag statements whose per-call **cumulative** latency is **bimodal/multimodal**. Only tuning flag is **`--min-calls`**. For delta analysis, interactive filtering, and dip-confirmed offline tiers, use the viewer's **Latency modes** tab (with `watch --snapshot-latency-analysis`). Requires the optional `[histogram]` extra. |
...
**Snapshot tuning:** **`--interval`** (seconds between watch ticks, default `60`); **`--ash-window-minutes`** (rolling ASH window when `--ash-start`/`--ash-end` are not set, default `5`); or fixed **`--ash-start`** / **`--ash-end`** (ISO-8601). **`--snapshot-statements-per-node`** (default `200`) and **`--snapshot-ash-per-node`** (default `1000`) cap how many rows are stored per node in each file. **`--node-parallelism`** (default `8`) limits how many nodes are queried concurrently during each snapshot (useful on large clusters). **`--snapshot-ash-top-tables`** (default `25`, `0` disables) ranks **`table_id`** values cluster-wide by ASH samples after per-node collection. **`--snapshot-table-ddl`** (opt-in) fetches **YSQL** `CREATE TABLE` / `CREATE INDEX` DDL for those top tables (via the seed connection). **`--snapshot-latency-analysis`** (opt-in; implies `--snapshot-latency-histograms`) additionally precomputes the **dip-confirmed** multimodality report into an **`ybtop.latency.*.json`** sidecar per snapshot so the browser can show confirmed tiers offline without shipping any statistics code (best-effort; skipped with a log note if the `[histogram]` extra is not installed). **`--snapshot-latency-histograms`** (opt-in) collects each statement's **`yb_latency_histogram`** as a separate top-N section (**`--snapshot-latency-histograms-per-node`**, default `100`, ranked by call count) for the `ybtop histogram` command and the viewer's **Latency modes** tab. **`--snapshot-retention-hours`** (default `3`, `0` disables) prunes old snapshot files and the manifest. **`--output-dir`** (default current directory) is the same path you pass to **`serve --data-dir`**.

**What latency multimodality finds:** Some queries have a **split personality**; most calls are fast, but a meaningful chunk are much slower (or there are several distinct speed "clusters" rather than one). Averages, and even p99s, hide this: a single number can't tell you that a query is really behaving like *two or three different queries at once*. `ybtop histogram` (and the viewer's **Latency modes** tab) reads YugabyteDB's per-statement latency histogram and looks for exactly that shape — statements whose per-call latency splits into two or more separate **speed classes** (e.g. a fast ~2 ms mode and a slow ~50 ms mode). Those are the queries worth a closer look, because the split usually has a concrete, fixable cause: tablet **leader vs. follower/local reads**, buffer **cache hit vs. miss**, **read-restart** or **lock-conflict retries**, **cross-AZ vs. same-AZ** hops, a **leader move** during the window, or the optimizer **flipping query plans**. Each flagged statement is reported with a **confidence tier**, the number of **peaks** (speed classes found), the overall latency **spread**, and the **gap** between the fast and slow modes; near-identical statements are collapsed into recurring **query templates** so one shared root cause shows up once instead of many times. Run it on **cumulative** totals for a quick "is anything split right now?" check, or on the **delta** between two snapshots to catch a split that *just started* before a large historical baseline dilutes it away.

**Latency multimodality (`ybtop histogram`)**: Reads snapshot files only (no database) and reports the **latest** snapshot's **cumulative** totals. **`--min-calls`** (default `30`) ignores low-traffic statements; rows are FDR-corrected (Benjamini-Hochberg) and shown at the **`high`** confidence tier and above. Without the `diptest` package the confirmatory dip test is skipped and shape-flagged rows are reported as **`unconfirmed`** (the same behavior as the browser viewer). Delta analysis, tier/flagged filtering, and dip-confirmed offline tiers live in the viewer's **Latency modes** tab; capture snapshots with **`--snapshot-latency-analysis`** to precompute the sidecars it reads.
